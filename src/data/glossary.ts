...
초보자는 지정가 주문을 습관화하는 게 좋아. 시장가로 급하게 사면 슬리피지 + 높은 수수료로 손해볼 수 있거든. 특히 유동성 낮은 알트코인은 반드시 지정가로!`,
  ...
  // ===== 추가 트레이딩 용어 =====
  {
    ...
    titleKo: '펀딩비',
    titleEn: 'Funding Rate',
    shortDef: '무기한 선물 시장에서 롱/숏 간 주기적으로 교환되는 수수료.',
    explanation: `펀딩비(Funding Rate)는 무기한 선물(Perpetual Futures) 가격을 현물 가격에 수렴시키기 위한 메커니즘이야.

**작동 원리:**
- 펀딩비 양수(+): 롱이 숏에 지불 → 시장이 과열됐다는 신호
- 펀딩비 음수(-): 숏이 롱에 지불 → 시장이 과도하게 비관적

**정산 주기:**
- 거래소마다 다름: 1시간(Hyperliquid), 4시간, 8시간(Binance)
- 정산 시점에 포지션을 보유하고 있어야 수수료 주고받음

**차익거래 활용:**
두 거래소의 펀딩비 차이를 이용해 수익을 내는 전략이 펀딩비 차익거래야. 한쪽에서 펀딩비를 받고, 반대쪽에서 헷지하는 구조.`,
    category: '트레이딩',
    relatedTermSlugs: ['leverage', 'delta-neutral', 'open-interest'],
    relatedStrategySlugs: ['funding-rate-arbitrage'],
  },
  {
    slug: 'delta-neutral',
    titleKo: '델타 뉴트럴',
    titleEn: 'Delta Neutral',
    shortDef: '가격 변동 리스크를 0으로 만드는 헷지 전략.',
    explanation: `델타 뉴트럴은 같은 자산을 롱과 숏 동시에 보유해서 가격 변동에 영향을 받지 않는 상태를 말해.

**원리:**
- A거래소에서 BTC 롱 $1,000
- B거래소에서 BTC 숏 $1,000
- BTC가 오르면: 롱 +$100, 숏 -$100 = 순손익 $0
- BTC가 내리면: 롱 -$100, 숏 +$100 = 순손익 $0

**수익은 어디서?**
가격 변동이 아닌 다른 요소에서 수익을 추구해:
- 펀딩비 차이
- 베이시스(선물-현물 가격차)
- 유동성 공급 수수료

**주의점:**
완벽한 델타 뉴트럴은 이론적이야. 실제로는 수수료, 슬리피지, 청산 리스크 등으로 약간의 차이가 발생해.`,
    category: '트레이딩',
    relatedTermSlugs: ['funding-rate', 'leverage', 'liquidation'],
    relatedStrategySlugs: ['funding-rate-arbitrage'],
  },
  {
    slug: 'liquidation',
    titleKo: '청산',
    titleEn: 'Liquidation',
    shortDef: '마진이 부족해지면 거래소가 포지션을 강제로 닫는 것.',
    explanation: `청산(Liquidation)은 레버리지 포지션의 손실이 예치한 마진(증거금)을 초과할 위험이 있을 때, 거래소가 자동으로 포지션을 닫는 거야.

**예시:**
- $1,000 마진 + 10x 레버리지 = $10,000 포지션
- 가격이 10% 역방향으로 이동 → 손실 $1,000 = 마진 전액
- 거래소가 강제 청산 → 마진 전액 손실

**연쇄 청산(Cascade):**
대규모 청산이 발생하면 시장에 큰 매도/매수 압력 → 가격이 더 이동 → 추가 청산 유발. 이런 "폭포수 효과"가 급락/급등의 주요 원인이야.
...
- 변동성 높은 시기에는 포지션 축소`,
    category: '트레이딩',
    relatedTermSlugs: ['leverage', 'funding-rate', 'open-interest'],
    relatedStrategySlugs: ['liquidation-hunting'],
  },
  {
    slug: 'open-interest',
    titleKo: '미결제약정',
    titleEn: 'Open Interest (OI)',
    shortDef: '아직 청산되지 않은 선물/옵션 계약의 총 수.',
    explanation: `미결제약정(OI)은 현재 시장에서 열려있는 선물 포지션의 총 규모를 나타내.

**OI가 중요한 이유:**
- OI 상승 + 가격 상승 = 새 자금 유입, 강한 상승세
- OI 상승 + 가격 하락 = 숏 포지션 증가, 하락 압력
- OI 급감 = 대규모 청산 또는 포지션 정리

**활용법:**
- 높은 OI = 큰 움직임(청산 폭포) 가능성
- OI 집중 가격대 = 청산 레벨 (지지/저항)
- OI + 펀딩비 조합 분석이 가장 유용

**확인 도구:**
CoinGlass, Coinalyze, Hyperliquid 등에서 무료로 확인 가능.`,
    category: '트레이딩',
    relatedTermSlugs: ['funding-rate', 'liquidation', 'leverage'],
    relatedStrategySlugs: ['liquidation-hunting', 'funding-rate-arbitrage'],
  },
  {
    slug: 'mev',
    titleKo: 'MEV',
    titleEn: 'Maximal Extractable Value',
    shortDef: '블록 생성자가 트랜잭션 순서를 조작해 추출하는 이익.',
    explanation: `MEV(Maximal Extractable Value)는 블록을 만드는 사람이 트랜잭션 순서를 바꾸거나 삽입해서 얻는 추가 수익이야.

**대표적인 MEV 유형:**
- **샌드위치 공격**: 큰 거래 앞뒤로 주문을 끼워넣어 차익 확보
- **프론트러닝**: 큰 매수 주문 전에 먼저 사서 가격 차이로 수익
- **청산**: 청산 가능한 포지션을 먼저 발견해 청산 보상 확보
- **차익거래**: DEX 간 가격 차이를 이용

**일반 사용자에게 미치는 영향:**
DEX에서 큰 금액을 스왑할 때 불리한 가격에 체결될 수 있어 (샌드위치 공격). 슬리피지 설정을 낮게 유지하고, MEV 보호 기능이 있는 DEX를 사용하는 게 좋아.`,
    category: '온체인',
